...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-loadblock=<file>", _("Imports blocks from external reet000??.dat file on startup"));
    ...
    strUsage += HelpMessageOpt("-reindex", _("Rebuild chain state and block index from the reet*.dat files on disk"));
#ifndef WIN32
...
{
    const std::string URL_SOURCE_CODE = "<https://gitlab.com/reflect/reflect>";
    const std::string URL_WEBSITE = "<https://reetcash.com/>";
    ...
    return FormatParagraph(strprintf("Copyright (C) %i-%i The Bitcoin Core Developers", 2009, COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf("Copyright (C) %i-%i The Reflect Developers", 2014, 2018)) + "\n" +
           "\n" +
           FormatParagraph(strprintf("Copyright (C) %i-%i The Reflect Core Developers", 2018, COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Please contribute if you find Reflect Core useful. "
...
void CleanupBlockRevFiles()
{
    ...
    // Glob all reet?????.dat and rev?????.dat files from the blocks directory.
    // Remove the rev files immediately and insert the reet file paths into an
    // ordered map keyed by block file index.
    LogPrintf("Removing unusable reet?????.dat and rev?????.dat files for -reindex with -prune\n");
    ...
    for (directory_iterator it(blocksdir); it != directory_iterator(); it++) {
        ...
        {
            if (it->path().filename().string().substr(0,3) == "reet")
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    if (fReindex) {
        ...
        while (true) {
            ...
            if (!boost::filesystem::exists(GetBlockPosFilename(pos, "reet")))
                break; // No block files left to reindex
            FILE *file = OpenBlockFile(pos, true);
            if (!file)
                break; // This error is logged in OpenBlockFile
            LogPrintf("Reindexing block file reet%05u.dat...\n", (unsigned int)nFile);
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Reflect Core version %s\n", FormatFullVersion());
}
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Upgrading to 0.8; hard-link the old reetnnnn.dat files into /blocks/
    ...
    if (!boost::filesystem::exists(blocksDir))
    {
        ...
        for (unsigned int i = 1; i < 10000; i++) {
            boost::filesystem::path source = GetDataDir() / strprintf("reet%04u.dat", i);
            if (!boost::filesystem::exists(source)) break;
            boost::filesystem::path dest = blocksDir / strprintf("reet%05u.dat", i-1);
            try {
                ...
                LogPrintf("Error hardlinking reet%04u.dat: %s\n", i, e.what());
                break;
            }
        }
        if (linked)
