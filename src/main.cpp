...
#if defined(NDEBUG)
# error "Reflect cannot be compiled without assertions."
#endif
...
CAmount GetProofOfWorkSubsidy()
{
    ...
    if (nBlockHeight == 1) { return 50000 * COIN;
    }  
    if (nBlockHeight <= 200000) { return 8 * COIN;
    }
    if (nBlockHeight <= 400000) { return 7 * COIN;
    }
    if (nBlockHeight <= 600000) { return 6 * COIN;
    }
    if (nBlockHeight <= 800000) { return 5 * COIN;
    }
}
...
CAmount GetProofOfStakeSubsidy()
{
    ...
    if (nBlockHeight <= 200000) { return 2 * COIN;
    }
    if (nBlockHeight <= 400000) { return 3 * COIN;
    }
    if (nBlockHeight <= 600000) { return 4 * COIN;
    }
    return 5 * COIN;
}
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (nSigOpsCount > MAX_BLOCK_SIGOPS)
            return state.DoS(100, error("ConnectBlock(): too many sigops"),
                             REJECT_INVALID, "bad-reet-sigops");
    ...
    if (!fKnown) {
        ...
        if (nNewChunks > nOldChunks) {
            ...
            if (CheckDiskSpace(nNewChunks * BLOCKFILE_CHUNK_SIZE - pos.nPos)) {
                ...
                if (file) {
                    LogPrintf("Pre-allocating up to position 0x%x in reet%05u.dat\n", nNewChunks * BLOCKFILE_CHUNK_SIZE, pos.nFile);
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.vtx.empty() || block.vtx.size() > MAX_BLOCK_SIZE || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION) > MAX_BLOCK_SIZE)
        return state.DoS(100, false, REJECT_INVALID, "bad-reet-length", false, "size limits failed");
    ...
    if (nSigOps > MAX_BLOCK_SIGOPS)
        return state.DoS(100, false, REJECT_INVALID, "bad-reet-sigops", false, "out-of-bounds SigOpCount");
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex=NULL, bool fProofOfStake=true)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (mi == mapBlockIndex.end())
            return state.DoS(10, error("%s: prev block not found", __func__), 0, "bad-prevreet");
        pindexPrev = (*mi).second;
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK)
            return state.DoS(100, error("%s: prev block invalid", __func__), REJECT_INVALID, "bad-prevreet");
...
void UnlinkPrunedFiles(std::set<int>& setFilesToPrune)
{
    for (set<int>::iterator it = setFilesToPrune.begin(); it != setFilesToPrune.end(); ++it) {
        ...
        boost::filesystem::remove(GetBlockPosFilename(pos, "reet"));
        boost::filesystem::remove(GetBlockPosFilename(pos, "rev"));
        LogPrintf("Prune: %s deleted reet/rev (%05u)\n", __func__, *it);
    }
}
...
void FindFilesToPrune(std::set<int>& setFilesToPrune, uint64_t nPruneAfterHeight)
{
    ...
    LogPrint("prune", "Prune: target=%dMiB actual=%dMiB diff=%dMiB max_prune_height=%d removed %d reet/rev pairs\n",
...
FILE* OpenBlockFile(const CDiskBlockPos &pos, bool fReadOnly) {
    return OpenDiskFile(pos, "reet", fReadOnly);
}
...
bool static LoadBlockIndexDB()
{
    ...
    // Check presence of reet files
    LogPrintf("Checking all reet files are present...\n");
...
bool LoadExternalBlockFile(const CChainParams& chainparams, FILE* fileIn, CDiskBlockPos *dbp)
{
    ...
    try {
        ...
        CBufferedFile reetdat(fileIn, 2*MAX_BLOCK_SIZE, MAX_BLOCK_SIZE+8, SER_DISK, CLIENT_VERSION);
        uint64_t nRewind = reetdat.GetPos();
        while (!reetdat.eof()) {
            boost::this_thread::interruption_point();

            reetdat.SetPos(nRewind);
            nRewind++; // start one byte further next time, in case of failure
            reetdat.SetLimit(); // remove former limit
            unsigned int nSize = 0;
            try {
                // locate a header
                unsigned char buf[MESSAGE_START_SIZE];
                reetdat.FindByte(chainparams.MessageStart()[0]);
                nRewind = reetdat.GetPos()+1;
                reetdat >> FLATDATA(buf);
            ...
            try {
                // read block
                uint64_t nBlockPos = reetdat.GetPos();
                if (dbp)
                    dbp->nPos = nBlockPos;
                reetdat.SetLimit(nBlockPos + nSize);
                reetdat.SetPos(nBlockPos);
                CBlock block;
                reetdat >> block;
                nRewind = reetdat.GetPos();
