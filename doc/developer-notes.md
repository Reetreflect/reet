...
- [Development guidelines](#development-guidelines)
    - [General Reflect Core](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Reflect Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
Reflect Core is a multi-threaded application, and deadlocks or other
...
Reflect Core can be compiled with various "sanitizers" enabled, which add
...
- [ThreadImport (`b-loadreet`)](https://doxygen.bitcoincore.org/init_8cpp.html#ae9e290a0e829ec0198518de2eda579d1)
  : Loads blocks from `reet*.dat` files or `-loadblock=<file>` on startup.
...
However, in open source software such as Reflect Core, where everyone uses
...
pay attention to for reviewers of Reflect Core code.

General Reflect Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Reflect Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Reflect Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Reflect Core so that they can be integrated
...
file descriptors, it will cause problems with Reflect's `select()` loop, because
...
compatibility between nodes. This happened in Reflect 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Reflect's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Reflect Core, the wallet was always
