...
def template_to_bytearray(tmpl, txlist):
    reetver = pack('<L', tmpl['version'])
    mrklroot = genmrklroot(list(dblsha(a) for a in txlist))
    timestamp = pack('<L', tmpl['curtime'])
    nonce = b'\0\0\0\0'
    reet = reetver + a2b_hex(tmpl['previousblockhash'])[::-1] + mrklroot + timestamp + a2b_hex(tmpl['bits'])[::-1] + nonce
    reet += varlenEncode(len(txlist))
    for tx in txlist:
        reet += tx
    return bytearray(reet)
...
class GetBlockTemplateProposalTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_template(node, tmpl, txlist, 'inconclusive-not-best-prevreet')
